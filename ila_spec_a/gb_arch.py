...
class GBArch ():
    def __init__ (self):
        ...
        self.RAM = []
        self.RAM_size = Y_EXTEND - 1 # 8
        for i in xrange (0, self.RAM_size):
            buffName = 'RAM_%d' % i
            self.RAM.append (m.mem (buffName, COL_ADDR_SIZE, DATA_SIZE)) # 9 * 8
        ...
        self.stencil = []
        self.slice_size = Y_EXTEND * DATA_SIZE # 9 * 8
        self.stencil_size = X_EXTEND # 9
